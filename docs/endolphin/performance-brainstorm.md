...
- **mfm-js の直接ネイティブ化は見送り**: 最大の純 JS CPU だが、パース結果の完全互換が必須（1 ノードのズレで mentions・連合 HTML が変わる = P3 リスク）のため本家が動かない限り fork 単独では非推奨。先に安い代替として **FeedService のリクエスト毎再パースのキャッシュ化**（ノート単位の HTML キャッシュ or ETag/304。ノートは不変なので挙動不変で実現可能）[効果:小〜中 / コスト:小 / 乖離:低] → **C-15 (#110)**
- **（2026-07-03 更新）mfm.kt 起点の Rust 移植構想は起票**: mfm-js 互換の Kotlin 実装 [mfm.kt](https://github.com/samunohito/mfm.kt)（自作）は型設計・構文分解（NodeFactory 31 個 + 型付きノード階層）が完了しており、Rust への写像はほぼ機械的。言語中立 JSON コーパスの 3 実装共有 + fuzz 差分で P3 リスクに答え、NAPI 境界は extract/toHtml 特化 API で回避、配布は slacc 同型の optionalDependencies + mfm-js フォールバック。既知の壁は regex 方言（lookaround）・UTF-16/UTF-8 境界・Unicode 絵文字判定の 3 点 [効果:中 / コスト:大（外部プロジェクト） / 乖離:低] → **C-16 (#111)**
