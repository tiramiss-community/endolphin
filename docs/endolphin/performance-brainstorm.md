...
- **mfm-js のネイティブ化は見送り**: 最大の純 JS CPU だが、パース結果の完全互換が必須（1 ノードのズレで mentions・連合 HTML が変わる = P3 リスク）のため本家が動かない限り fork 単独では非推奨。先に安い代替として **FeedService のリクエスト毎再パースのキャッシュ化**（ノート単位の HTML キャッシュ or ETag/304。ノートは不変なので挙動不変で実現可能）[効果:小〜中 / コスト:小 / 乖離:低] → **C-15 (#110)**
