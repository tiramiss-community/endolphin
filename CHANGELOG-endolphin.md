...
- Enhance: フロントエンドのビルド成果物 (`built/_frontend_vite_/`) を軽量化（サポート言語は不変）。`LocaleInliner` が言語非依存チャンクを推移閉包まで含めて判定し、安全な部分集合（約 104 チャンク）を `scripts/` に 1 本化して各言語ディレクトリへの重複コピーを廃止（locale 固有チャンク内の参照は静的/動的 import を `../scripts/` へ、`__vite__mapDeps` リテラルは `scripts/` 据え置きへ向け直す）。さらに全 JS チャンクを Brotli (quality 11) で事前圧縮した `.js.br` を生成し、既定では元 `.js` を削除（ビルドターゲット safari18.2 / chrome130 / firefox132 はすべて Brotli 対応で、JS チャンクは常にブラウザが `Accept-Encoding: br` 付きで取得するため）。`loader/boot.js`（HTML インライン展開）と `mockServiceWorker.js` は元 `.js` を温存。環境変数 `FRONTEND_KEEP_RAW_JS=true` で元 `.js` をフォールバック用に残すことも可能。成果物は約 181MB → 約 53MB に縮小
