...
| `login(page, name, pass)` | upstream の `data-testid` 経由の UI ログイン。signin 後の home リロード競合を避けるため、`signin-flow` 応答 + home 要素の可視まで待つ |
...
1. **`data-testid` 再利用 (最優先)。** upstream が自分の Playwright e2e 用に保守している属性を `getByTestId(...)` で使う。例: `signin` / `signin-username` / `open-post-form` / `post-form-text` / `post-form-submit` / `user-setup-dialog` / `modal-window-close` / `modal-dialog-ok`。
2. **role / text (`data-testid` が無い画面)。** `getByRole('menuitem', { name: 'Timeline' })` / `getByText(...)`。rig は `locale: 'en-US'` 固定なので英語テキストで決定的に当たる (`playwright.config.ts` 参照)。
3. **安定 CSS class (最後の手段)。** フォロー の `.koudoku` など、意味が安定しているクラスのみ。codegen が吐く脆い nth-child / 長い CSS パスはここに**頼らない**。

新たに `data-testid` を upstream UI に足したくなったら、それは `packages/frontend/` の改変＝ upstream churn を増やす行為。原則は**既存 `data-testid` の再利用**に留め、無ければ role/text で凌ぐ (fork-policy P2 の追従コスト最小化)。
